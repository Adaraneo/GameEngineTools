...
// [DESIGN SIMPLIFICATION] OceanFraction=0.71 matches TerraGen's own PlanetNoise.ContinentSeaBias calibration target, not an Earth-only assumption -- every planet this codebase's TerraGen generates trends toward that land/sea split regardless of Universe config. Still no actual per-point geography (uniform ocean fraction at every latitude) -- a real 2D geography-resolved model (North, Mengel & Short 1983) is out of scope.
public static class SeasonalTemperatureAmplitudeModel
{
    ...
    private const double OceanFraction = 0.71;
    ...
    /// <summary>Seasonal temperature-swing amplitude (°C, half peak-to-peak) at this latitude — area-weighted blend of the ocean and land thermal-response amplitudes (NOT a blend of the heat capacities themselves, which would misrepresent the nonlinear per-surface-type response).</summary>
    public static double AmplitudeC(PlanetSettings.Resolved planet, double latDeg)
    {
        ...
        return OceanFraction * oceanAmplitude + (1.0 - OceanFraction) * landAmplitude;
    }
