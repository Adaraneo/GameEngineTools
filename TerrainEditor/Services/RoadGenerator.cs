...
/// <summary>
/// Terrain-aware pathfinding between two world-space points: A* over the heightmap grid,
/// penalizing slope (roads avoid climbing steep terrain), river crossings (a moderate
/// "ford/bridge" cost), and below-sea-level cells (a much steeper cost — a road/path
/// crossing open sea makes far less sense than fording a river, so it's avoided unless
/// there's truly no way around). None of these are hard blocks. Used to turn a straight-line
/// <c>Connection</c> into a road that actually follows the land.
/// </summary>
public static class RoadGenerator
{
    private const double SlopePenaltyFactor = 4.0; // higher = roads bend harder to avoid steep terrain
    ...
    /// exists because slope/river costs are penalties, not hard blocks.
    /// </summary>
