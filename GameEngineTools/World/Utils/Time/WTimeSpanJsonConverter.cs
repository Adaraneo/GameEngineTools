...
namespace GameEngineTools.World.Utils.Time
{
    public sealed class WTimeSpanJsonConverter : JsonConverter<WTimeSpan>
    {
        ...
        private bool TryParseGeneral(string s, out WTimeSpan span)
        {
            ...
            if (partsD.Length == 2)
            {
                if (!long.TryParse(partsD[0], out days))
                {
                    return false;
                }
            ...
            if (!long.TryParse(parts[0], out var hh))
            {
                return false;
            }

            if (!long.TryParse(parts[1], out var mm))
            {
                return false;
            }

            string[] secSub = parts[2].Split('.', 2);
            if (!long.TryParse(secSub[0], out var ss))
            {
                return false;
            }
            ...
            if (secSub.Length == 2)
            {
                if (!long.TryParse(secSub[1], out subticks))
                {
                    return false; // bereme raw subticks (v jednotce worldTicku)
                }
            }

            // validace v rámci spec
            var spec = _wtctx.Spec;
            if (hh < 0 || hh >= spec.HoursPerDay)
            {
                return false;
            }

            if (mm < 0 || mm >= spec.MinutesPerHour)
            {
                return false;
            }

            if (ss < 0 || ss >= spec.SecondsPerMinute)
            {
                return false;
            }

            if (subticks < 0 || subticks >= spec.TicksPerSecond)
            {
                return false;
            }
