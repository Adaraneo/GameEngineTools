...
namespace GameEngineTools.Characters.Generation
{
    ...
    public sealed class AppearanceGenerator : IAppearanceGenerator
    {
        ...
        public PhysicalAppearance Generate(SexBiology sex, int seed, StadiumType stadium = StadiumType.Adult, AppearanceGenSpec? spec = null)
        {
            ...
            var ms = spec.Morphology ?? MorphologyGenerationSpec.For(stadium, sex);
        ...
        private static FaceLatent GenerateFaceLatent(MorphologyGenerationSpec ms, BodyLatent b, IRandomSource rng)
        {
        ...
        private static FacialMorphology GenerateFace(BodyMorphology body, MorphologyGenerationSpec ms, FaceLatent l, IRandomSource rng)
        {
        ...
        #endregion Face generation

        #region Surface and labels

        private static SurfaceTraits GenerateSurface(StadiumType stadium, MorphologyGenerationSpec ms, FaceLatent face, BodyLatent body, IRandomSource rng)
        {
            ...
            return new SurfaceTraits(
                ...
                ScarProbability: R01(stadium == StadiumType.Baby ? 0.01 : 0.03 + age * 0.08 + ms.SurfaceDetailRate * 0.08),
                WrinkleTendency: R01(0.04 + age * 0.72 + (1.0 - body.Posture) * 0.06),
                AgeSurfaceFactor: R01(age));
        }

        private static ColorTraits GenerateColors(AppearanceGenSpec spec, IRandomSource rng)
            => new(
        ...
        private static double Normal(IRandomSource rng) => rng.NextUnit() + rng.NextUnit() + rng.NextUnit() - 1.5;
        private static double J(IRandomSource rng, double a) => Normal(rng) * a;
        private static double Lerp(double t, double a, double b) => a + (b - a) * t;
        private static double Clamp(double v, double min, double max) => v < min ? min : (v > max ? max : v);
        private static double C01(double v) => Clamp(v, 0.0, 1.0);
        private static double ClampSigned(double v) => Clamp(v, -1.0, 1.0);
        private static double R1(double v) => Math.Round(v, 1);
        private static double R3(double v) => Math.Round(v, 3);
        private static double R01(double v) => Math.Round(C01(v), 3);
        ...
        private sealed record BodyLatent(double HeightCm, double HeightNorm, double Robustness, double Vertical, double Horizontal, double Muscularity, double Adiposity, double LowerMass, double SoftTissueFullness, double Posture, double Juvenility, double Aging, double SexDimorphism);
        private sealed record FaceLatent(double Robustness, double Softness, double MidfaceProjection, double Angularity, double BrowProminence, double NoseScale, double EyeScale, double LipFullness, double Juvenility, double Aging, double SexDimorphism);
